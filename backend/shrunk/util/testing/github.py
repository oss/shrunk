...
def pull_outlook_assets_from_github():
    ...
    assets_response = requests.get(asset_url, headers={"Accept": "application/vnd.github+json",
                                                    "X-GitHub-Api-Version": "2022-11-28"})
    ...
    for url in asset_zip_urls:
        ...
        print("Unzipping {asset} to {destination}".format(
            asset=asset,
            destination=extraction_folder_dest
        ))
        # shutil.unpack_archive(asset, os.path.join(var_folder, folder))
        #unzip folder and move to /var_folder/folder
        os.system("unzip {asset} -d {destination}".format(
            asset=asset,
            destination=extraction_folder_dest
        ))
