...
const EndpointChart: React.FC<{ endpointData: EndpointDatum[] | null }> = (props) => {
    ...
    const options = {
        ...
        xAxis: { categories: props.endpointData.map(datum => datum.endpoint), title: { text: 'Endpoint' } },
        yAxis: { min: 0, title: { text: 'Visits' }, labels: { overflow: 'justify', step: 4 } },
        legend: {
            layout: 'vertical',
            align: 'right',
            verticalAlign: 'top',
...
export class AdminStats extends React.Component<Props, State> {
    ...
    updateEndpointData = async (): Promise<void> => {
        await fetch('/api/v1/admin/stats/endpoint')
            .then(resp => resp.json())
            .then(json => this.setState({ endpointData: json.stats as EndpointDatum[] }));
    }

    submitRangeForm = async (values: { range: moment.Moment[] | null | undefined }): Promise<void> => {
        const range = values.range;
        const adminDataRange = (range === undefined || range === null) ? null : {
            begin: range[0],
            end: range[1],
        };
            ...
            <>
                ...
                <Row className='primary-row'>
                    <Col span={24}>
                        ...
                        <Row>
                            {this.state.adminData === null ? <Spin size='small' /> :
                                <>
                                    <span className='info'>
