...
$('.toggle-admin-button').click((event) => {
    ...
    const member_netid = $(event.target).closest('.member-row').attr('data-member-netid');
    const member_is_admin = $(event.target).closest('.member-row').attr('data-member-is-admin');
